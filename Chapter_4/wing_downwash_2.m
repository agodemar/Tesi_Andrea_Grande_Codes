...
k_lambda = 1.257;
k_h = 1.031;
d_espilon_zero = 4.44*(k_ar*k_lambda*k_h)^1.19;
d_espilon = d_espilon_zero*(1-mach*mach)^(1/2);
lambda_C4 = 0*u.rad;

%% Write data file
[status, msg] = mkdir("./wing_downwash_2"); % create folder first
fid = fopen('./wing_downwash_2/data.tex', 'w');
if (fid == -1)
    ...
    fprintf(fid, "\\def\\myDownwashGradientAtMachZeroHTail{%f}\n", d_espilon_zero);
    fprintf(fid, "\\def\\myDownwashGradientHTail{%f}\n", d_espilon);
    fprintf(fid, "\\def\\mySweepQuarterChordWingRAD{%f}\n", lambda_C4);
    fprintf(fid, "\\def\\myXrWingMACQuarterChordToHTailMACQuarterChordMT{%f}\n", x_h_w);
    fprintf(fid, "\\def\\myZrWingMACQuarterChordToHTailMACQuarterChordMT{%f}\n", h_w);
    fprintf(fid, "\\def\\myFactorKARDownwashGradientHTail{%f}\n", k_ar);
    fprintf(fid, "\\def\\myFactorKmrDownwashGradientHTail{%f}\n", k_h);
    fprintf(fid, "\\def\\myFactorKlambdaDownwashGradientHTail{%f}\n", k_lambda);
