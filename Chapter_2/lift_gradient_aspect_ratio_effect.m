...
cl_alpha_med =( 2/(wing_surface/u.m^2))* integral(fun,0,10.72);
...
%% Write data file
[status, msg] = mkdir("./lift_gradient_aspect_ratio_effect"); % create folder first
fid = fopen('./lift_gradient_aspect_ratio_effect/data_b.tex', 'w');
if (fid == -1)
...
coeff_b_cl = cl_alpha_r_rad;
fun=@(y) (((coeff_a_cl_radm*u.m)*y+coeff_b_cl).*((coeff_a_c)*y+coeff_b_c/u.m));
cl_alpha_med =( 2/(wing_surface/u.m^2))* integral(fun,0,16.08);
cl_alpha_med_rad = cl_alpha_med*u.rad;
cl_alpha_med_deg = cl_alpha_med*u.deg;
oswald_factor = 0.90;
cl_alpha = cl_alpha_med_rad/(1+(cl_alpha_med_rad/(pi*oswald_factor*aspect_ratio)));
...
%% Write data file
[status, msg] = mkdir("./lift_gradient_aspect_ratio_effect"); % create folder first
fid = fopen('./lift_gradient_aspect_ratio_effect/data_c.tex', 'w');
if (fid == -1)
    fprintf("Cannot open file.")
else
    fprintf(fid, "\\def\\mySpanWingMT{%f}\n", b);    
